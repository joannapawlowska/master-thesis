...
przydzielonej procesowi czy ilością wykorzystanych wątków. Analiza oparta jest na danych zgromadzonych
...
Podczas przeprowadzania testów wydajnościowych istotne jest zachowania podobnych warunków środowiska testowego.
...
było zastosowanie uśredniania wyników. Dla każdej konfiguracji parametrów program został uruchamiany kilkukrotnie,
aby podczas generowania wykresów, rezultaty mogły zostać uśrednione, stając się przez to bardziej wiarygodne.
Ręczne uruchamianie programu z wiersza poleceń jest mało optymalne, dlatego cały proces zbierania rezultatów testów
został zautomatyzowany. W celu wielokrotnego uruchamiania programu dla różnych parametrów, zostały utworzone
...
atrybutów będzie wzrastała proporcjonalnie, nie jest łatwym zadaniem. Z tego powodu, kolejny wykres przestawia zależności tylko dla
...
Powyższy rysunek daje powody do stwierdzenia, że żadna ze stworzonych implementacji równoległych nie jest optymalna,
...
Z powodu braku możliwości dołożenia pamięci RAM w komputerze, na którym przeprowadzane były testy, zdecydowano, że
...
Kolejnym typem omawianej pamięci jest pamięć nazywana stertą (\textit{Heap Space}). Jest to wydzielona
część pamięci RAM, która przydzielona jest maszynie wirtualnej Javy w czasie jej startu.
Podczas uruchomiania programu, napisanego w języku Java, istnieje możliwość konfiguracji ilości
...
maksymalna wartość \text{Heap Size} wynosi 4096 megabajtów. Powiększanie \textit{Heap Size} ma sens, jeśli do pamięci programu
...
przy niskiej wartości \textit{Heap Size}. Nieznaczące pogorszenie efektywności działania programu, wraz ze wzrostem wartości \textit{Heap Size},
może wynikać z tego, że uruchomianie maszyny wirtualnej Javy z większą ilością przydzielonej pamięci, jest bardziej czasochłonnym procesem.

\subsubsection{Liczba wątków}
Ostatnim przeanalizowanym zagadnieniem jest wpływ ilości tworzonych wątków na czas trwania
pracy programu. Podczas tej analizy, pojęcie wątek używane jest w kontekście wątków
tworzonych w ramach maszyny wirtualnej Javy. Każdy wątek tworzony przez \textit{JVM} związany jest
...
Liczba wątków, którą należy użyć w programie Java, zależy od wielu czynników. Największe znaczenie
mają zasoby sprzętowe, charakter programu czy wielkość obciążenia programu.
...
Z kolei zbyt duża ilości wątków, negatywnie wpływa na czas wykonania. Przyczyną takiej tendencji, 
są ograniczone zasoby systemu operacyjnego, który umożliwia ustaloną ilość operacji, które
efektywnie mogą być przetwarzane równolegle. Powoływanie do życia nowego wątku
jest kosztowną operacją, dlatego tworzenie wątków w zbyt dużej ilości, może negatywnie wpłynąć
...
potrzebnego na wykonanie. Z tego powodu, w celu poprawy wydajności wprowadzono poprawki
...
implementacji, które mogą powodować wycieki pamięci tzw. 'Memory Leaks'. Wycieki powodują przepełnienie pamięci,
...
w trakcie trwania wykonywania programu.
...
z czasem trwania programu, a \textit{Garbage Collector} nie miał możliwości zwalniania jest dostatecznie szybko. 
Najprawdopodobniej kluczową role w zaistniałym problemie odgrywa wykorzystanie rekurencji, która nie jest optymalna pod względem
zajmowanej pamięci. Dodatkowym czynnikiem może być ładowanie do pamięci programu ogromnych zbiorów danych.
Próbą zmniejszenia wykorzystanej pamięci było zaimplementowanie współdzielenia danych. Zamiast duplikacji części rekordów,
