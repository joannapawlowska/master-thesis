...
Istnieje wiele wzorców programowania równoległego. Podczas implementacji algorytmu równoległego ciężko jest dobrać jeden pasujący wzorzec.
Wzorce traktowane są raczej jako ogólne wskazówki, które mogą być przydatne podczas projektowania algorytmu. Najczęściej programy
...
Wzorzec inaczej nazywany jest wzorcem Manager-Worker. Procesor, który nazywany jest zarządcą (Master/Manager)
...
Wzorzec Fork-Join to jeden z najczęściej stosowanych podejść. Program rozpoczyna pracę jako jeden wątek główny.
W momencie, gdy w kodzie programu pojawia się instrukcja, która wymaga równoległego przetwarzania, tworzone są dodatkowe wątki,
które wykonywane są równolegle.
Dopóki wszystkie wątki nie zakończą pracy i nie zostaną zniszczone, wątek główny nie może zacząć wykonywać kolejnej części kodu.
Etap fork polega na ustawieniu argumentów, które następnie otrzymuje każdy wątek.
Etap join łączy wyniki po zakończeniu pracy wszystkich wątków równoległych \cite{parallel-design-patterns}.
...
W działaniu obydwa wzorce są prawie identycznie, jednak wzorce wywodzą się z różnych pomysłów.
% Chociaż są one strukturalnie identyczne, rodzaj wykonywanej pracy odzwierciedla nieco inną filozofię.
...
lub gdy istotnie różnią się złożonością. Zadania przechowywane są w strukturze danych, która dostępna jest w pamięci współdzielonej.
Najczęściej wykorzystywane struktury to lista, kolejka priorytetowe czy tablica z haszowaniem. W momencie gdy procesor zakończył
obliczanie zadania, dostarczane jest mu kolejne zadanie przechowywane w strukturze.

\subsubsection{Wzorzec Pipeline}
Wzorzec Pipeline polega na potokowym przetwarzaniu danych. Nazywany jest również metodą producent i konsumenta.
Strumień danych przekazywany jest do kolejnych procesorów, które modyfikują oryginalny strumień.
Każdy z procesorów wykonuje na danych inny rodzaj obliczeń, które należą do etapu potoku.
Dane wyjściowe jednego etapu stają się danymi wejściowymi kolejnego etapu.
Stosowanie wzorca jest efektywne, jeśli czasy realizacji poszczególnych etapów są podobne \cite{wprowadzenie-do-obliczen-rownoleglych}.
...
\begin{thebibliography}{6}
    ...
    \bibitem{parallel-design-patterns} OpenCSF Project [Online]. Available: \url{https://w3.cs.jmu.edu/kirkpams/OpenCSF/Books/csf/html/ParallelDesign.html}. Dostęp: 12.12.2022
