...
wykorzystuje równoległość do obliczania zrównoważonego zysku informacji dla poszczególnych atrybutów.
...
\subsection{Implementacje algorytmu ID3}

Jako narzędzie został wybrany język programowania Java, dlatego implementacja opiera się na podejściu
obiektowym. Pierwotnie program został zaimplementowany z wykorzystaniem uproszczeń jakie oferuje język Java 11
...
\subsection{Wspólne fragmenty implementacji}
...
Przy tworzeniu równoległej wersji algorytmy zastosowano podejście, które umożliwia dziedziczenie klas. Taka technika umożliwia wykorzystanie
...
zysku informacji w węźle jest równa 0. Listing \ref{lst:sequential-tree} przedstawia rekurencyjną metodę, która
umożliwia rozbudowę drzewa decyzyjnego do momentu, gdy warunek stopu zostanie spełniony.
...
Pierwsza implementacja z zastosowaniem równoległości polega na podziale zadań znajdowania zrównoważonego przyrostu
informacji pomiędzy oddzielne wątki. Jest to podejście które wykorzystuje równoległość danych. Dane dzielone są pionowo (wertykalnie).
Na potrzeby tej implementacji została stworzona klasa \verb|ParallelDecisionTable|, która rozszerza klasę \verb|DecisionTable|.
Została w niej nadpisana tylko jedna metoda, której celem jest odnalezienie atrybutu dzielącego węzeł -- \verb|public int findAttributeToDivideBy()|.
W metodzie tworzona jest lista zadań \verb|List<ComputeGainRatioTask> taskList|. Zadanie obliczenia zysku informacji dla jednego atrybutu zdefiniowane jest w
klasie \verb|ComputeGainRatioTask|, która implementuje interfejs \verb|Callable|. Klasa \verb|ExecutorService| udostępnia metodę, która jako parametr
przyjmuje listę zadań, które mają zostać wykonane równolegle. Podczas tworzenia instancji klasy \verb|ExecutorService| należy również zdefiniować
ilość wątków, które mają wykonać listę zadań.
Implementacja opiera się na wzorcu Work Pool \ref{work-pool}. Zadania z listy przekazywane są wolnym wątkom do momentu, aż wszystkie zadania zostaną rozdzielone
i wykonane. Istotny jest fakt, iż dostęp do rezultatów jest dostępny dopiero wtedy, gdy wszystkie wątki zakończą prace. W momencie, gdy
wszystkie wyniki zgromadzone zostaną w formie listy, wybierany jest ten atrybut, którego zrównoważony zysk informacji jest największy.
...
Program został sparametryzowany, tak aby mógł został uruchomiony w każdym wariancie implementacji algorytmu ID3 oraz dla różnych zbiorów danych.
Argumenty pozwalają na ustawianie następujących wartości:
...
\begin{lstlisting}[caption=Przykładowe uruchomienie programu w wersji sekwencyjnej,frame=single]
    java -jar concurrent-id3.jar
        --data-set C:/resources/data.csv\
        --delimiter ,\
        --output-file C:/results.txt\
        --threads 5\
        --test-ratio 0.1\
        --train-ratio 0.9\
        --type SEQUENTIAL
